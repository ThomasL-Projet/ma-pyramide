<?php
// titre de la page
$this->assign('title', 'Ma Pyramide Santé - Rapport Evolution : details alimentaires');
...
<nav class="breadcrumbs" role="menubar" aria-label="breadcrumbs">
    ...
    <li role="menuitem" class="current"><?php echo $this->Html->link('Détails alimentaires', 'Javascript:void(0);'); ?></li>
</nav>
<div id="presentation">
    <div class="row">
        <div class="small-12 small-centered columns">
            <div class="title-area"> Détails alimentaires </div> 

            <div class="textarea">
                <p class="text-justify">
                    Voyez le groupe alimentaire et le contenu nutritionnel de vos aliments chaque jour
                </p>			
            </div>
        </div>
    </div>
    <div class="row">
        <div class="small-12 columns">
            <?php if ($affichage) : ?>
                ...
                <div id="detailali" >
                    ...
                    foreach ($suivis as $suivi) :
                        ...
                        if (!empty($suivi['Suivialimentaire']['id_aliment']))
                            $passe[] = $suivi['Suivialimentaire']['id_aliment'];
                        else
                            $passePerso[] = $suivi['Suivialimentaire']['id_horsclass'];
                        if (!empty($suivi['Suivialimentaire']['id_aliment']))
                            $portionpasse[] = $suivi['Suivialimentaire']['nomPortion'];
                        else
                            $portionpassePerso[] = $suivi['Suivialimentaire']['nomPortion'];
                        ...
                        if (!empty($alim[$i]['Aliment']))
                            echo "<h3>" . $this->Html->link($alim[$i]['Aliment']['Aliment']['nomFR'], '../app/webroot/img/imagesAliment/' . $fichier, array('class' => 'zoombox', 'alt' => $alim[$i]['Aliment']['Aliment']['nomFR'], 'escape' => true)) . "</h3>";
                        else
                            echo "<h3>" . $suivis[$i]['Alimhorsclassification']['nom'] . '</h3>';

                        if (!empty($alim[$i]['Aliment']))
                            echo '<p><strong>Groupe alimentaire</strong> : ' . $alim[$i]['Aliment']['Famillealiments']['name'] . '</p>';
                        else
                            echo '<p><strong>Type</strong> : Cet aliment est un aliment hors classification</p>';
                ...
                ?><div style="font-style:italic">Navigation : </div><?php echo $this->Paginator->prev('<<' . __('Précédent', true), array(), null, array('class' => 'disable')); ?>
